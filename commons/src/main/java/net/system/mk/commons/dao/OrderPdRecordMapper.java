...
public interface OrderPdRecordMapper extends BaseMapper<OrderPdRecord> {
    ...
    @Select("select pi.id,pi.product_name,pi.pic_1_url,pi.pic_2_url,pi.pic_3_url,pi.label_price,opr.order_price,opr.commission,pi.product_desc,pi.special_offer,opr.order_no from order_pd_record opr inner join order_record ord on ord.order_no = opr.order_no inner join product_info pi on ord.product_id = pi.id  where pd_status = 0 and mbr_id = #{mbrId} limit 1")
    ...
    @Select("select opr.*,pi.product_name,pi.pic_1_url,pi.pic_2_url,pi.pic_3_url,pi.label_price,pi.product_desc,pi.special_offer from order_pd_record opr inner join order_record ord on ord.order_no = opr.order_no inner join product_info pi on ord.product_id = pi.id where opr.mbr_id = #{mbrId} and opr.pd_status = #{pdStatus}")
