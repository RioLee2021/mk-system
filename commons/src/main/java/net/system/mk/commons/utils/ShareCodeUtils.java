...
public class ShareCodeUtils {
	 ...
	 * 进制长度
	 */
	private static final int BIN_LEN = BASE_CHAR.length;
	 ...
	 * long转换为邀请码
	 */
	public static String encodeToCode(long id) {
		...
		buf[--charPos] = BASE_CHAR[(int) (id % BIN_LEN)];
		// 将字符数组转化为字符串
		...
		if (len < CODE_LEN) {
			...
			for (int i = 0; i < CODE_LEN - len - 1; i++) {
				sb.append(BASE_CHAR[random.nextInt(BIN_LEN)]);
			}
			result += sb.toString();
		}
		return result;
	 ...
	 * 邀请码解析出id
	 */
	public static Long decodeToCode(String code) {
		char[] charArray = code.toCharArray();
		long result = 0L;
		for (int i = 0; i < charArray.length; i++) {
			int index = 0;
			for (int j = 0; j < BIN_LEN; j++) {
				if (charArray[i] == BASE_CHAR[j]) {
					index = j;
					break;
				}
			}
	...
	public static void main(String[] args) {
		System.out.println(encodeToCode(System.currentTimeMillis()));
	}
